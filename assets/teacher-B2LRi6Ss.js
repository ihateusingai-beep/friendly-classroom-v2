import{c as d,g as p,r as b,b as v,e as c,a as m}from"./index-78Fjzb1d.js";const u={};function h(){return`
  ...
  `}function y(){const i=p();return i.length?`
    <div class="container fade-in">
      ...
      <div class="teacher-panel">
        <h2>👥 學生總覽</h2>
        <div class="subtitle">共 ${i.length} 位學生</div>
      </div>

      <div class="fc-flex-col-gap" role="list" aria-label="學生清單">
        ${i.map(t=>{const a=v(t.name),e=a.score,o=a.completedCount,l=u[t.name]||"👤",n=e>=200?"🌟":e>=100?"⭐":e>=50?"✨":"💫";return`
          <div class="student-row" role="listitem" aria-label="學生 ${c(t.name)}，完成 ${o} 個場景，最近 ${t.lastPlayed||"—"}，道德分 ${e}，等級 ${n}">
            <span class="avatar" aria-hidden="true">${l}</span>
            <span class="info">
              <span class="name">${c(t.name)}</span>
              <span class="meta">完成 ${o} 個場景 · 最近 ${t.lastPlayed||"—"}</span>
            </span>
            <span class="stat-badge">
              <span class="num" style="color:${e>=100?"#15803d":"var(--text)"}">${e}</span>
              <span class="label">道德分</span>
            </span>
            <span style="font-size:1.2em" aria-hidden="true">${n}</span>
          </div>`}).join("")}
      </div>
      ...
      <div class="card" class="fc-mt-16">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px" role="list" aria-label="科目總覽" class="subject-grid">
          ${m().map(t=>{const a=i.reduce((l,n)=>{var s,r;return l+(((r=(s=n.subjectProgress)==null?void 0:s[t.id])==null?void 0:r.completed)||0)},0),e=i.reduce((l,n)=>{var s,r;return l+(((r=(s=n.subjectProgress)==null?void 0:s[t.id])==null?void 0:r.total)||0)},0),o=e?Math.round(a/e*100):0;return`
              <div style="background:${t.color}18;border:2px solid ${t.color};border-radius:12px;padding:12px;text-align:center" role="listitem"
                aria-label="${t.title}，全班完成 ${a}/${e} 題，${o}%">
                <div style="font-size:1.5em;margin-bottom:4px" aria-hidden="true">${t.emoji}</div>
                <div style="font-weight:700;font-size:1.1em;color:${t.color}">${a}/${e}</div>
                <div style="height:6px;background:#eee;border-radius:3px;margin-top:6px;overflow:hidden" role="progressbar"
                  aria-valuenow="${o}" aria-valuemin="0" aria-valuemax="100" aria-label="${t.title} 全班進度">
                  <div style="height:100%;width:${o}%;background:${t.color};border-radius:3px"></div>
                </div>
              </div>`}).join("")}
        </div>
      </div>
  ...
  `:`
    <div class="container fade-in">
      ...
      ${b({emoji:"📭",title:"暫時沒有學生數據",hint:"學生完成學習後會自動顯示在這裡"})}
      <div class="fc-mt-16">
        <button type="button" class="btn btn-outline" data-action="goRoleSelect">← 返回首頁</button>
      </div>
      ${d()}
