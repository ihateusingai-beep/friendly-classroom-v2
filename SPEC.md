...
## 22.17 Sprint 26 (2026-06-26) — 情緒小偵探 pedagogy 中度智障 (MID) 適配

**目標**：10 個 emotion-detective scenarios 之中 5 個（ed-3/5/7/9/10）嘅 correct answer 對 MID 學生嚟講 ambiguous, 因為：

1. 答案 label pool imbalance — 驚 vs 驚訝 FAU overlap > 60%, audio prosody 差 pitch 0.10 過細
2. Abstract social emotion（尷尬 / 困惑 / 驕傲）對 K1-K3 + MID 認知階段太抽象
3. Scenario wording 唔夠 cue, 例如 ed-5「朋友大叫嘩！」中性, 無 threat/joke 之分

**5 個 scenario 改動**：

| ed | 原 correct | 新 correct | 改動理由 |
|---|---|---|---|
| ed-3 比人搶玩具 | 嬲 | **喊** | MID 學生 first reaction = 喊 (傷心/委屈),「嬲」要 cognitive appraisal「對方係故意」先 trigger, 太 mature. Wording 加「小明好傷心，眼淚都流晒出嚟」強化 cue |
| ed-5 朋友大叫 | 驚訝 (unchanged) | 驚訝 | Wording 由「嘩！」中性 interjection 改做「生日快樂！仲送咗一份禮物」= unambiguously positive surprise, 排除 threat interpretation |
| ed-7 全班望住 | 尷尬 | **驚** | 全班望住對 MID 學生 = exposure anxiety = 驚 (怯場),「尷尬」對佢哋太抽象 |
| ed-9 唔識答老師問題 | 困惑 | **驚** | Wording 加「小明好驚畀老師鬧」= fear-of-failure cue.「困惑」對 MID 學生太抽象,「驚」concrete |
| ed-10 考試攞第一 | 驕傲 | **開心** | 「驕傲」要 self-evaluative emotion + cognitive maturity, K1-K3 MID 學生唔識表達.「開心」concrete + observable |

**Test invariant 改動 (`tests/sprint25-emotion-categories.test.js` §4)**：

- ❌ 舊：`Ekman 6 basic emotions are all present as correct answers` (6 distinct)
- ✅ 新：`Ekman 6 basic emotions all exposed across pool` (correct OR distractor, 容許重複)
- ❌ 舊：`social category covers 4 distinct self-evaluative / social emotions`
- ✅ 新：`social category covers 4 scenarios` (social-context trigger, emotion label 可為 basic Ekman)

**Pedagogy 原理**：

對 MID 學生嚟講，重複 exposure (ed-2 + ed-3 都係 喊) 透過 spaced repetition 鞏固 vocabulary label，比 strict Ekman 6 distinct mandate 更 work。Social context 可以 trigger basic emotion — 重點係 context 嘅 social nature, 而非 label 必然係 self-evaluative emotion。

**Face image reuse**：所有 30 張 face PNG 沿用, 因為同一角色風格一致, 唔使 regen (Sprint 23 character anchor)。Face image 嘅 reuse mapping：

| Scenario | Correct face (新) | 來源 |
|---|---|---|
| ed-3 | `ed-3-face-crying.png` | 原 distractor 升 correct |
| ed-7 | `ed-7-face-scared.png` | 原 distractor 升 correct |
| ed-9 | `ed-9-face-scared.png` | 原 distractor 升 correct |
| ed-10 | `ed-10-face-happy.png` | 原 distractor 升 correct |

**Scenario image (context 插圖)**：

- ed-5 `ed-5-scenario.png` 要 regen — context cue 由 neutral 大叫變 positive birthday surprise, AI gen 重做
- 其他 4 個 scenario image 維持, 因為 scenario context 描述同 visual cue 一致 (例如 ed-7 「全班望住」對 驚 仍然 match)

**Version bump**: 2.9.0 → **2.10.0** (MINOR — pedagogy 改動, 答案變更)

**Tests**：`npm test` 全部 PASS, sprint25 test invariant 改動後 5 個 it() block 維持綠燈

**Acceptance criteria**：
- [x] 5 個 scenario wording + correct answer + emotionLabel 改動 apply
- [x] `tests/sprint25-emotion-categories.test.js` §4 invariant 由 Ekman 6 distinct → at-least-once pool exposure
- [x] `package.json` version 2.9.0 → 2.10.0
- [x] `npm test` PASS (zero regression)
- [x] `BUILD_LOG.md` prepend v2.10.0 entry
- [ ] `ed-5-scenario.png` regen with positive surprise context (pending — AI gen step)
